...
hard stop on every pull request (PRs #50, #51, #52 and #54 all had to be admin-merged for
...
a review that actually ran, so real findings are never discarded for a weaker opinion.

The verdict is deliberately narrower than the primary's. Ollama constrains decoding to the
schema, so the output *shape* is guaranteed — a stronger guarantee than the path it backs
up — but the *judgments* are not. It assesses only what it can ground in the diff, reports
the documentation-contract fields as unevaluated, and the gate titles the result
...
a PR cannot alter the workflow that reviews it; the job holds no secrets and never executes
repository code (the diff reaches the model as text); and the runner is ephemeral and
containerised, taking one job before being destroyed. When the runner is offline the job
